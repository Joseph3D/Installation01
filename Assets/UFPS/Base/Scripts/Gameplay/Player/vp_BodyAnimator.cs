...
public class vp_BodyAnimator : MonoBehaviour
{
	...
	protected virtual void UpdateBody()
	{
		...
		// make lower body smoothly face forward when moving
		if ((m_CurrentHeadLookYaw > FeetAdjustAngle)
			|| (m_CurrentHeadLookYaw < -FeetAdjustAngle)
			|| (Player.Velocity.Get().magnitude > 0.1f))
		{
	...
	protected virtual void UpdateSpine()
	{
		...
		for (int v = 0; v < m_HeadLookBones.Count; v++)
		{
			...
			if ((Player.IsFirstPerson.Get()
				// TIP: uncomment this to allow the camera to look at the arms when standing
				// still and unarmed (but beware: looks  stiffer, and glitchy in crouch mode)
	...
	void InitMaxSpeeds()
	{
		...
		// TEMP: for now, get max speed of the first vp_FPController
		// that we can find in the scene
		vp_FPController controller = FindObjectOfType<vp_FPController>();
		if (controller != null)
		{
	...
	/// 
	/// </summary>
	void NotInSameHierarchyError(GameObject o)
	{
