...
public class vp_RagdollHandler : MonoBehaviour
{
	...
	protected CharacterController m_CharacterController = null;		// unity charactercontroller, NOTE: only used on local player
	...
	public virtual void SetRagdoll(bool enabled = true)
	{
		...
		// disable/enable any components that conflict with ragdoll physics
		if(Animator != null)			Animator.enabled = !enabled;
		if(BodyAnimator != null)		BodyAnimator.enabled = !enabled;
		...
		// if this call was to disable ragdoll physics, restore the
		// initial state of all the rigidbodies
		if (!enabled)
			RestoreStartPose();
	...
	protected virtual void OnStart_Dead()
	{
		...
		// have the momentum of the player controller carry over
		// into the initial velocity of the rigidbodies
		foreach (Rigidbody r in Rigidbodies)
		{
